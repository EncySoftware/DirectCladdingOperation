...
public class Build : NukeBuild
{
    ...
    private IBuildSpace InitBuildSpace()
    {
        ...
        var settings = new SettingsObject
        {
            ...
            {
                Path.Combine(RootDirectory.Parent, "project", "main", ".stbuild", "DirectCladdingOperationExtensionProject.json")
            },
    ...
    private Target Compile => _ => _
        .Executes(() =>
        {
            ...
            foreach (var project in _buildSpace.Projects)
            {
                ...
                // copy xml file
                var sourceXmlPath = Path.Combine(mainProjectFolder, "DirectCladdingOperation_ExtOp.xml");
                if (!File.Exists(sourceXmlPath))
                    throw new Exception($"{sourceXmlPath} file not found");
                var targetXmlPath = Path.Combine(dllFolder, "DirectCladdingOperation_ExtOp.xml");
    ...
    private Target Pack => _ => _
        ...
        .Executes(() =>
        {
            foreach (var project in _buildSpace.Projects)
            {
                ...
                // additional files
                var xmlOperation = Path.Combine(dllFolder, "DirectCladdingOperation_ExtOp.xml");
